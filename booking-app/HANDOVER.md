...
booking-app/
  src/
    ...
    components/DentalBookingWidget.astro   funnel widget: calendar -> time -> confirm only
    components/GeneralBookingWidget.astro  same, for the general funnel
    pages/book.astro                 hosts the 4-step widget (direct/legacy entry point)
    pages/book/dental.astro          hosts DentalBookingWidget  (dental funnel lands here)
    pages/book/general.astro         hosts GeneralBookingWidget (general funnel lands here)
    ...
    pages/api/csb/
      ...
      book.ts                        POST rate-limit -> honeypot -> validate -> duplicate
                                      ...
                                      -> fire-and-forget GHL stage update
      ghl-lead.ts                    POST funnel submission -> GoHighLevel contact
                                      (+ opportunity, dental only)
...
### The qualification funnels and GoHighLevel

Most real traffic no longer starts at `/book/`. Two questionnaire funnels sit in front of it,
qualify the lead, route it to a provider, and push it to GoHighLevel. This is the part of the
system with the most moving pieces and the least obvious ownership, so read this before
touching either funnel.

**The funnels are hand-written HTML in the *marketing site's* `public/`, not Astro pages and
not part of this project:**

```
public/start/index.html          general funnel   -> /book/general/
public/dental/start/index.html   dental funnel    -> /book/dental/
public/start/confirmed/          post-booking pages
public/dental/start/confirmed/
```

Two consequences that have both caused bugs:

1. They are served from the **committed `dist/`** of the marketing site, so editing them does
   nothing until the root site is rebuilt and redeployed — a separate build from this one.
2. Their answer values are duplicated in `ghl-lead.ts`'s label maps and in their own routing
   rules. Nothing enforces that they agree. They have already drifted twice: an ad-spend map
   left mapping retired values, and a routing rule still testing spend buckets the form no
   longer emits.

**Flow.** Questionnaire → route decision → `POST /api/csb/ghl-lead/` (fire-and-forget) →
redirect to `/book/{dental,general}/?provider=<key>`, with the answers handed over in
`sessionStorage` (`dental_lead` / `site_lead`). The booking widget reads that object for
contact details, so a visitor arriving without it cannot book; both widgets detect this up
front and link back to the funnel.

**Routing** is a revenue/ad-spend threshold. Qualified leads go to Cade, under-threshold to
Keith as a partner referral. The decision is made *client-side in the funnel HTML* and sent
as `route`; `ghl-lead.ts` only trusts it to pick tags and the "referred to" field.

**`ghl-lead.ts`** creates or merges a GHL contact, tags it (`dental-qualified` + `cade-direct`,
or `dental-under-threshold` + `keith-referral`), and maps every answer onto GHL custom fields
**by hardcoded field ID**. Those IDs are opaque and unversioned — if someone deletes or
recreates a custom field in GHL, the write silently stops landing with no error here. It also
stamps a source from the click IDs: `fbclid` → Facebook Ads, else `gclid` → Google Ads, else
organic. Dental leads additionally get an opportunity in the Dental Pipeline at "New Lead";
the general funnel deliberately creates no opportunity.

**Stage advance on booking.** `book.ts` calls `updateGhlOpportunityStage(email)` after a
successful `events.insert`, looking the contact up by email and moving their Dental Pipeline
opportunity to "Call Booked". It is fire-and-forget by design: a CRM failure must never fail
a booking that already exists on a real calendar.

**Known GHL API quirk.** The contacts endpoint rejects `notes` on create with
`422 property notes should not exist`; notes must be attached with a separate
`POST /contacts/{id}/notes` call. The opportunity-create payload still sends `notes` inline
and may fail the same way — its response is now logged so the next real submission settles it.

**Configuration.** `GHL_API_KEY` and `GHL_LOCATION_ID`. When either is unset the integration
returns early rather than throwing, so a misconfigured deploy loses CRM sync silently while
bookings keep working. Pipeline and stage IDs are constants at the top of `ghl-lead.ts` and
`book.ts`.
